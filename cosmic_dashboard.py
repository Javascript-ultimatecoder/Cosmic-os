...
def _run_evolution_cycle(cycle_size: int = 18) -> dict:
    global EVOLUTION_TICKS, TOTAL_BIRTHS, INTELLIGENCE_TIER, COMPANY_COUNT
    _init_god_state()
    ...
    EVOLUTION_TICKS += 1
    TOTAL_BIRTHS += births
    COMPANY_COUNT += random.randint(1, 5) + promotions // 2
...
<body>
    ...
    <div class="max-w-7xl mx-auto p-6 md:p-8 relative z-10">
        ...
        <section class="bg-black/50 border border-pink-600 rounded-3xl p-6 mt-8">
            <div class="flex items-center justify-between gap-4 mb-4">
                <h2 class="text-2xl text-pink-300">Power Leaderboard</h2>
                <button id="refresh-board" class="px-4 py-2 rounded-full border border-pink-300 text-pink-200 hover:bg-pink-400/10">Refresh Board</button>
            </div>
            <div id="leaderboard" class="grid gap-3 text-sm"></div>
        </section>
    ...
    <script>
        ...
        async function refreshStatus() {
            ...
            metaNode.textContent = `${data.audit_events} audit events stored • population=${data.population} • companies=${data.companies} • DB: ${data.audit_db}`;
        ...
        async function refreshLeaderboard() {
            ...
            const data = await res.json();
            leaderboardNode.innerHTML = data.items.map((row, idx) => `
                <div class="border border-pink-500/40 rounded-2xl p-3 bg-pink-500/5">
                    <div class="text-pink-300 font-bold">#${idx + 1} ${row.name}</div>
                    <div class="text-pink-100/80">${row.rarity.toUpperCase()} • Tier ${row.tier} • Power ${row.power} • Offspring ${row.offspring}</div>
                </div>
            `).join('');
        }

        document.getElementById('refresh-feed').addEventListener('click', refreshStatus);
        document.getElementById('refresh-board').addEventListener('click', refreshLeaderboard);
...
async def leaderboard(limit: int = 10) -> dict:
    ...
    items = sorted(
        GOD_STATE.values(),
        key=lambda g: (-g["power"], -g["tier"], RARITY_ORDER.index(g["rarity"]), g["name"]),
    )[:limit]
    return {"items": items, "population": len(GOD_STATE), "companies": COMPANY_COUNT}


@app.get("/genealogy/{god_name}")
async def genealogy(god_name: str) -> dict:
    _init_god_state()
    god = GOD_STATE.get(god_name)
    if not god:
        raise HTTPException(status_code=404, detail="god not found")
    parent_rows = [GOD_STATE[parent] for parent in god.get("parents", []) if parent in GOD_STATE]
    return {
        "god": god,
        "parents": parent_rows,
        "has_lineage": bool(parent_rows),
        "population": len(GOD_STATE),
    }


@app.get("/economy")
async def economy() -> dict:
    return {
        "companies": COMPANY_COUNT,
        "population": len(GOD_STATE) if GOD_STATE else sum(PANTHEON_DISTRIBUTION.values()),
        "births": TOTAL_BIRTHS,
